...
constexpr size_t kHandleTableNumPages = (kHandleTableSize + BIT(
...
uint32_t get_new_base_value(void *p, uint32_t index)
{
    ...
    uint32_t new_gen = (((old_gen + 1) << kHandleGenerationShift) &
...
void init_handle_table(vspace_t *vspace)
{
    ...
    void *handle_pool = vspace_new_pages_with_config(vspace, &config,
                    seL4_AllRights);
